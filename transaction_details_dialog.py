from PyQt6.QtWidgets import (QDialog, QVBoxLayout, QHBoxLayout, QLabel, QPushButton, 
                           QTextBrowser, QTabWidget, QWidget, QTextEdit, QDialogButtonBox)
from PyQt6.QtCore import Qt
from datetime import datetime

class TransactionDetailsDialog(QDialog):
    def __init__(self, tx_data, address=None, transaction=None, parent=None):
        ...
        if transaction and transaction.block_number:
            block_info = QLabel(
                f"Block Number: {transaction.block_number}\n"
                f"Block Time: {transaction.block_time.strftime('%Y-%m-%d %H:%M:%S UTC')}"
            )
            block_info.setStyleSheet("font-weight: bold;")
            summary_layout.addWidget(block_info)
            summary_layout.addWidget(QLabel(""))  # Spacer
        
        # Transaction hash
        if tx_data.get('txid'):
            hash_label = QLabel(f"Transaction Hash: {tx_data['txid']}")
            hash_label.setTextInteractionFlags(Qt.TextInteractionFlag.TextSelectableByMouse)
            summary_layout.addWidget(hash_label)
        
        # Add other transaction details...
        if tx_data.get('confirmations'):
            summary_layout.addWidget(QLabel(f"Confirmations: {tx_data['confirmations']}"))
        
        if tx_data.get('size'):
            summary_layout.addWidget(QLabel(f"Size: {tx_data['size']} bytes"))
        
        if tx_data.get('vsize'):
            summary_layout.addWidget(QLabel(f"Virtual Size: {tx_data['vsize']} vBytes"))
